...
impl CountBytes for Message {
  fn count_bytes(&self) -> usize {
    let size = match self {
      ...
      Self::ChannelTimeRangeRequest { ttl, channel, time_start, time_end, limit, .. } => {
        varint::length(4) + 4 + varint::length(*ttl as u64)
          + varint::length(channel.len() as u64) + channel.len()
          + varint::length(*time_start) + varint::length(*time_end)
          + varint::length(*limit as u64)
      },
      Self::ChannelStateRequest { ttl, channel, limit, updates, .. } => {
        varint::length(5) + 4 + varint::length(*ttl as u64)
          + varint::length(channel.len() as u64) + channel.len()
          + varint::length(*limit as u64) + varint::length(*updates as u64)
      },
...
impl ToBytes for Message {
  ...
  fn write_bytes(&self, buf: &mut [u8]) -> Result<usize,failure::Error> {
    ...
    Ok(match self {
      ...
      Self::HashRequest { req_id, ttl, hashes } => {
        offset += req_id.write_bytes(&mut buf[offset..])?;
  ...
  fn from_bytes(buf: &[u8]) -> Result<(usize,Self),failure::Error> {
    ...
    Ok(match msg_type {
      ...
      4 => {
        ...
        //offset += s;
        (msg_len as usize, Self::ChannelTimeRangeRequest {
          req_id, ttl: ttl as usize, channel, time_start, time_end, limit: limit as usize
        })
      },
      5 => {
        ...
        (msg_len as usize, Self::ChannelStateRequest {
          req_id, ttl: ttl as usize, channel, limit: limit as usize, updates: updates as usize
        })
      },
      6 => {
        if offset+4 >= buf.len() { failure::bail!["unexpected end of ChannelStateRequest"] }
        ...
        let (_,limit) = varint::decode(&buf[offset..])?;
        //offset += s;
        (msg_len as usize, Self::ChannelListRequest {
          req_id, ttl: ttl as usize, limit: limit as usize
        })
      },
      msg_type => (msg_len as usize, Self::Unrecognized { msg_type })
    })
  }
}
